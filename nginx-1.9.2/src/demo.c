...
：热代码替换，暂时估计是用于在不重启Nginx的情况下进行软件升级

NGX_PROCESS_JUST_RESPAWN标识最终会在ngx_spawn_process()创建worker进程时，将ngx_processes[s].just_spawn
= 1，以此作为区别旧的worker进程的标记。
*/
...
typedef struct {
  ...
  unsigned
      respawn : 1; // PROCESS_RESPAWN
                   // 创建进程的时候释放指定需要master拉起一个新的进程，如果进程挂了
...
void setproctitle(char *title) {
  ...
  p = strncpy((char *)os_argv[0], (char *)title, strlen(title));
  p += strlen(title);
...
void start_dispatcher_process(int type) {
  ...
  spawn_process(dispatcher_process_cycle, (void *)(intptr_t)0,
                "dispatcher process", type);
}

void save_argv(int argc, char *const *argv) { os_argv = (char **)argv; }
...
int reap_children(void) {
  ...
  for (i = 0; i < last_process; i++) { // 之前挂掉的进程
    ...
    fprintf(stderr,
            "child[%d] %d exiting:%d exited:%d detached:%d respawn:%d "
            "just_spawn:%d\n",
            i, processes[i].pid, processes[i].exiting, processes[i].exited,
            processes[i].detached, processes[i].respawn,
    ...
    if (processes[i].exited) {
      if (!processes[i].detached) {
        for (n = 0; n < last_process; n++) {
          if (processes[n].exited || processes[n].pid == -1) {
            continue;
          }
      ...
      if (processes[i].respawn // 需要重启进程
          && !processes[i].exiting && !terminate && !quit) {
        if (spawn_process(processes[i].proc, processes[i].data,
